...
Write-Host "  ICA Migration (Cross-Signed) - Step 8: Decommission Check" -ForegroundColor Cyan
...
if (-not $OutputDir) {
    ...
    $OutputDir = Join-Path $env:USERPROFILE "ica-crosssign-step8-$timestamp"
}
...
$report = @"
ICA Migration (Cross-Signed) - Decommission Readiness Report
