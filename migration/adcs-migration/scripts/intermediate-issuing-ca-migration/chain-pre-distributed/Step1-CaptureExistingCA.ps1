...
.PARAMETER CAName
    ...
    will enumerate all CA certs in the local store and prompt for selection.
...
Write-Host "  ADCS Migration - Step 1: Capture Existing CA Details" -ForegroundColor Cyan
...
if (-not $OutputDir) {
    ...
    $OutputDir = Join-Path $env:USERPROFILE "adcs-migration-step1-$timestamp"
}
...
if ($exportText -notmatch 'completed successfully') {
    Write-Host "[WARN] certutil -ca.cert failed. Trying alternate export..." -ForegroundColor Yellow
    # Try exporting from the Intermediate CA store directly
    if (Test-Path $certFile) { Remove-Item $certFile -Force }
    $exportResult = certutil -store CA $CAName $certFile 2>&1
    $exportText = ($exportResult | Out-String)
    if ($exportText -notmatch 'completed successfully') {
        # Last resort: export from My store (LocalMachine\My is where CA cert lives)
        Write-Host "[WARN] CA store export failed. Trying My store..." -ForegroundColor Yellow
        if (Test-Path $certFile) { Remove-Item $certFile -Force }
