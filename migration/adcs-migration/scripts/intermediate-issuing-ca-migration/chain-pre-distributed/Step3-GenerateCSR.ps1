...
.DESCRIPTION
    ...
    it in "pending cert install" state so that Step 6's certutil -installcert
...
.EXAMPLE
    .\Step3-GenerateCSR.ps1 -Step1OutputDir "C:\Migration\adcs-migration-step1-20260301"
...
Write-Host "  ADCS Migration - Step 3: Configure Subordinate CA & CSR" -ForegroundColor Cyan
...
if (-not $OutputDir) {
    ...
    $OutputDir = Join-Path $env:USERPROFILE "adcs-migration-step3-$timestamp"
}
...
# SubjectName here is the CA Common Name (just the CN value), not a full DN.
# Install-AdcsCertificationAuthority takes -CACommonName, not a subject DN.
$CACommonName = $SubjectName
if (-not $CACommonName) {
    # Extract CN from the old CA subject (e.g., "CN=HSB-RootCA" -> "HSB-RootCA")
...
if (-not (Test-Path $csrFile)) {
    ...
    Write-Host "        Install-AdcsCertificationAuthority did not produce a CSR." -ForegroundColor Red
    exit 1
}
...
Write-Host "  The CA is waiting for certutil -installcert with the" -ForegroundColor White
Write-Host "  signed certificate from the parent Root CA." -ForegroundColor White
Write-Host ""
...
Write-Host "  |  NEXT: Submit CSR to the ROOT CA for signing, then     |" -ForegroundColor Yellow
Write-Host "  |  bring the signed cert back and run Step 4.            |" -ForegroundColor Yellow
Write-Host "  +--------------------------------------------------------+" -ForegroundColor Yellow
Write-Host ""
