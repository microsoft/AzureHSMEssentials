<#
.SYNOPSIS
    Step 6: Validate the cross-signed Root CA configuration.

.DESCRIPTION
    Verifies that the new Root CA (installed in Step 3, with cross-cert
    published in Step 5) meets all requirements:

    Architecture (corrected):
    - The CA's IDENTITY certificate is self-signed (Subject == Issuer)
    ...
    - The cross-cert Issuer matches the OLD Root CA
    - The cross-cert AKI matches the OLD Root's SKI (chain proof)
    ...
    Key difference from Option 1: Option 1 has ONLY the self-signed cert.
    Option 2 has the self-signed cert PLUS the cross-cert trust bridge.

    RUN ON: NEW ADCS Server (after Step 5)

.PARAMETER OldRootCertPath
    Optional path to the OLD Root CA .cer file. If provided, verifies
...
.EXAMPLE
    .\Step6-ValidateCrossSignedCA.ps1 -OldRootCertPath "C:\temp\OldRootCA.cer"
#>
...
Write-Host "  Root CA Migration (Cross-Signed) - Step 6: Validate CA" -ForegroundColor Cyan
...
if (-not $OutputDir) {
    ...
    $OutputDir = Join-Path $env:USERPROFILE "rootca-crosssign-step6-$timestamp"
}
...
# -- Load old root cert if provided ------------------------------------------
$oldRootCert = $null
if ($OldRootCertPath -and (Test-Path $OldRootCertPath)) {
    ...
    Write-Host "       Thumbprint: $($oldRootCert.Thumbprint)" -ForegroundColor Gray
}
...
$certFile = Join-Path $OutputDir "NewRootCA-Identity.cer"
...
$dumpFile = Join-Path $OutputDir "NewRootCA-Identity-details.txt"
...
# -- Check 3: CA cert IS self-signed (Subject == Issuer) ---------------------
Write-Host ""
Write-Host "[3/11] Verifying CA cert is SELF-SIGNED (identity cert)..." -ForegroundColor White

$isSelfSigned = ($caCert.Subject -eq $caCert.Issuer)
Add-CheckResult -Name "CA Cert Self-Signed" -Passed $isSelfSigned `
    -Detail $(if ($isSelfSigned) { "Subject == Issuer (correct for Root CA identity)" } else { "Subject != Issuer! Root CA identity cert must be self-signed." })
...
if ($crossCandidates.Count -gt 0 -and $oldRootCert) {
    # DETERMINISTIC: Match by AKI = old CA's SKI (cryptographic proof of who signed it)
    $oldSKIHex = ""
    foreach ($ext in $oldRootCert.Extensions) {
...
# -- Check 6: Cross-cert Issuer matches OLD Root CA -------------------------
Write-Host ""
Write-Host "[6/11] Verifying cross-cert issuer is OLD Root CA..." -ForegroundColor White

if ($crossCert -and $oldRootCert) {
    $issuerMatchesCert = ($crossCert.Issuer -eq $oldRootCert.Subject)
    Add-CheckResult -Name "Cross-Cert Issuer = Old Root" -Passed $issuerMatchesCert `
        -Detail $(if ($issuerMatchesCert) { "Issuer matches: $($oldRootCert.Subject)" } else { "Cross Issuer: $($crossCert.Issuer) != Old Root: $($oldRootCert.Subject)" })
    ...
    Write-Host "  [SKIP] Old root cert not provided (-OldRootCertPath)" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[11/11] Checking Certificate Services and key accessibility..." -ForegroundColor White
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  Cross-Signed Root CA Validation Summary" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($allPassed) {
    ...
    Write-Host "  Step 6 Complete: Cross-signed CA validated successfully" -ForegroundColor Green
    Write-Host "============================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Architecture verified:" -ForegroundColor Green
    Write-Host "    - CA identity cert: self-signed (Subject == Issuer)" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "  Clients that trust the OLD root will chain through the" -ForegroundColor Green
    Write-Host "  cross-cert to trust the NEW root automatically." -ForegroundColor Green
    Write-Host ""
